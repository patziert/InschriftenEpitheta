...
for i in texts:
    ...
    j+=1
of = pandas.DataFrame(outputtexts)
csvdata = of.to_csv(encoding='utf-8', errors='replace' )
file = open("./Testdaten/kookinschriften.csv", "w", encoding='utf-8', newline='')
file.write(csvdata)
file.close()
print(zuschreibungen)
print(beziehungen)
print(kookkurrenz)
with open('/Testdaten/preresults.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['Begriff', 'Anzahl_Gesamt'])
    for key, value in zuschreibungen.items():
        writer.writerow([key, value])
    writer.writerow(['', ''])
    for key, value in beziehungen.items():
        writer.writerow([key, value])